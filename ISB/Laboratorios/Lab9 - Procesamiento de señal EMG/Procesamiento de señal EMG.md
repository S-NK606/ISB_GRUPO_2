...
El estudio realizado sobre el filtrado de señales EMG mediante el uso de un filtro wavelet en múltiples capas ha demostrado ser altamente eficaz en la supresión del ruido, especialmente en las frecuencias altas, mientras preserva los componentes de baja frecuencia. Esto resulta en señales limpias que permiten identificar claramente los momentos de activación y relajación muscular. 

Además, el análisis de la Mean Frequency of the Line (MFL) muestra que esta característica se incrementa con la intensidad del esfuerzo, lo que indica un mayor reclutamiento de fibras de contracción rápida durante cargas fuertes. Por otro lado, se observó que la Myopulse Percentage Rate (MYOP) aumenta significativamente en condiciones de fuerte oposición, reflejando un esfuerzo sostenido y un incremento en la demanda muscular. Para el caso de la longitud de onda (WAMP) y la Sample Entropy (SampEn) también se revelan patrones importantes, evidenciando la complejidad de las señales EMG y la capacidad adaptativa del sistema neuromuscular. Para la  curtosis (Kurt) se muestra como un indicador valioso para detectar contracciones de alta intensidad, destacando la variabilidad y amplitud de la respuesta muscular.

Cabe destacar que los parámetros de actividad, como el Hjorth Parameter 2 y la Slope Sign Changes, indican un aumento en la dinámica de la contracción muscular con el esfuerzo, lo cual sugiere que los músculos responden adecuadamente a diferentes estímulos que afectan la funcionalidad muscular en diversas condiciones. 
